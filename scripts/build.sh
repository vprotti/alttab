#!/bin/bash
# Builds the universal AltTab.app into dist/.
#
...
mkdir -p "$DIST"
lipo -create "$BIN_ARM/AltTab" "$BIN_X86/AltTab" -output "$DIST/AltTab-universal"
ARCHS="$(lipo -archs "$DIST/AltTab-universal")"
...
iconutil -c icns "$ICONSET" -o "$DIST/AltTab.icns"

echo "==> Assembling AltTab.app"
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$DIST/AltTab-universal" "$APP/Contents/MacOS/AltTab"
cp "$DIST/AltTab.icns" "$APP/Contents/Resources/AltTab.icns"
