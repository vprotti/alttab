...
RW_DMG="$DIST/AltTab-rw.dmg"
# Fail loudly: a silent 1.0.0 fallback would republish an old version number.
APP_VERSION="$(defaults read "$DIST/AltTab.app/Contents/Info" CFBundleShortVersionString)" \
  || { echo "error: could not read the version from dist/AltTab.app — run scripts/build.sh first" >&2; exit 1; }
OUT_DMG="$DIST/AltTab-$APP_VERSION.dmg"
VOL="/Volumes/AltTab"

[ -d "$DIST/AltTab.app" ] || { echo "error: dist/AltTab.app missing — run scripts/build.sh first" >&2; exit 1; }
...
NEWEST_SOURCE="$(find "$ROOT/Sources" "$ROOT/Support" -type f -newer "$DIST/AltTab.app/Contents/MacOS/AltTab" 2>/dev/null | head -1 || true)"
...
cp -R "$DIST/AltTab.app" "$STAGE/AltTab.app"
...
hdiutil create -volname "AltTab" -srcfolder "$STAGE" -format UDRW -fs HFS+ \
...
echo "==> Volume icon"
cp "$DIST/AltTab.icns" "$VOL/.VolumeIcon.icns"
swift "$ROOT/scripts/set-volume-icon.swift" "$VOL" "$DIST/AltTab.icns"
